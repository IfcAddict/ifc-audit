...
from ..core.selectors import select_ifc_ids, get_objects_from_ifc_ids
...
def fill_ui_tree(context, results):
    ...
    # Flat categories
    for key in ["orphans", "empty_psets", "unused_types", "duplicate_guids"]:
        ...
        for item in issues.get(key, []):
            ...
            if key == "orphans":
                ...
                if sev:
                    desc = f"{item.get('type', '')} | {sev}"
            elif key == "duplicate_guids":
...
class IFC_OT_run_audit(bpy.types.Operator):
    ...
    def execute(self, context):
        props = context.scene.ifc_auditor_props
        filepath = bpy.path.abspath(props.filepath).strip() if props.filepath else None
...
class IFC_OT_select_active_issue(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        context.scene.ifc_auditor_props.status = f"Selected {selected} objects for the active issue"
        return {'FINISHED'}
